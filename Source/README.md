...
```bash
mkdir bin
javac -d bin src\main\java\MTPIU\**\*.java
```
...
```bash
Chạy với Baseline MTPIU:
java -cp bin MTPIU.experiment.T10I4D100KBaselineMiner

Chạy với Standard MTPIU:
java -cp bin MTPIU.experiment.T10I4D100KStandardMiner

Chạy với Optimized MTPIU:
java -cp bin MTPIU.experiment.T10I4D100KOptimizedMiner
```
Testcase đề xuất:
- K = {750, 1000, 1250, 1500}
- α = {5, 6, 7, 8}
...
```bash
Chạy với Optimized MTPIU:
java -cp bin MTPIU.experiment.MushroomsOptimizedMiner
